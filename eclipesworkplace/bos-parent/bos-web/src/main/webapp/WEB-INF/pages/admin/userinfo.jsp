...
<script type="text/javascript">
	$(function(){
		...
		$('#save').click(function(){
			var v=$('#form').form("validate");//校验表单
				if(v){
					/* //还要获取被选中的角色id;
					 var arr = $("#roleTD input:checked");
					 var array = new Array;
					 //获得是页面元素是js格式的得 转成jq格式才可以用.val  或者使用js的getElementByName.value 来获取值
           ...
           <table class="table-edit"  width="95%" align="center">
				...
				<tr>
	           	...
	           	<tr><td>添加角色
	           		<script type="text/javascript">
	           			$(function(){
	           				//页面加载完之后，发送ajax请求动态的从数据库中查询 角色的列表
	           				var url='${pageContext.request.contextPath }/roleAction_listajax';
	           				$.post(url,function(data){
	           					for(var i=0;i<data.length;i++){
	           						$("#roleTD").append("<input id='"+data[i].id+"' type='checkbox' name='roleTD' value='"+data[i].id+"'><label for='"+data[i].id+"'>"+data[i].name+"</label><br/>")
	           					}
	           				},"json")
	           			})
	           		</script>
