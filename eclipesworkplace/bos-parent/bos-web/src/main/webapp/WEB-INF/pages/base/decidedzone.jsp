...
<script type="text/javascript">
	...
	function doAssociations(){
		...
		//alert(arr.length);
		//如果不是一个被选中，那么就提示只能选一个
		if(arr.length!=1){
			$.messager.alert("提示信息","请选择一个定区","warning");
		}
		else{
			//发送ajax请求向数据库要关联和没有关联的客户信息
			var url1="${pageContext.request.contextPath}/decidedzoneAction_findNotAssocition";
			...
			$.post(url1,function(data){
				...
				for(var i=0;i<data.length;i++){
					...
					var name=name+"[tel:"+telephone+"]";
					$("#noassociationSelect").append($("<option value='"+data[i].id+"'>"+name+"</option>"));
				}
			});
			
			var url2="${pageContext.request.contextPath}/decidedzoneAction_findHasAssocition";
			var id=arr[0].id;
	...
	function doDblClickRow(index,data){
		...
		$('#association_customer').datagrid( {
			...
			rownumbers : true,
			striped : true,
			url : "${pageContext.request.contextPath}/decidedzoneAction_findCustomerListByid?toCutomersid="+data.id,
			columns : [[{
				field : 'id',
				title : '客户编号',
				width : 120,
	...
	$(function(){
		$("#save").click(function(){
			var f=$("#adddecidezoneForm").form("validate");
			if(f){
...
<body class="easyui-layout" style="visibility:hidden;">
	...
	<div class="easyui-window" title="定区添加修改" id="addDecidedzoneWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		...
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="adddecidezoneForm" action="${pageContext.request.contextPath}/decidedzoneAction_add" method="post">
				<table class="table-edit" width="80%" align="center">
					...
					<tr height="300">
						...
						<td>
							<table id="subareaGrid"  class="easyui-datagrid" border="false" style="width:300px;height:300px" data-options="url:'${pageContext.request.contextPath}/subareaAction_listAjax',fitColumns:true,singleSelect:false">
								<thead>  
							        <tr>  
							...
							<script type="text/javascript">
								$(function(){
									...
									$("#associationBtn").click(function(){
										...
										//因为select里提交表单时候只会提交 被选中的  所以在提交之前把所有的select选中一下
