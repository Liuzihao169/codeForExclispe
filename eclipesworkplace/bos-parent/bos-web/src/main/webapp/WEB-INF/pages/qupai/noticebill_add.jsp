...
<script type="text/javascript">
	...
	$(function(){
		/////
		$("#customertelephone").focusout(function(){
			...
			$.post(url,{"telephone":telephone},
					function(data){
						if(data!=null){
							$("#noticebillForm input[name='customerId']").val(data.id);
							$("#noticebillForm input[name='customerName']").val(data.name);
							$("#noticebillForm input[name='delegater']").val(data.name);
							$("#noticebillForm input[name='pickaddress']").val(data.address);
						}else{
							$("#noticebillForm input[name='customerId']").val("");
							$("#noticebillForm input[name='customerName']").val("");
							$("#noticebillForm input[name='delegater']").val("");
							$("#noticebillForm input[name='pickaddress']").val("");
						}
	...
	<div region="center" style="overflow:auto;padding:5px;" border="false">
		<form id="noticebillForm" action="${pageContext.request.contextPath }/noticebillAction_add" method="post">
			<table class="table-edit" width="100%" align="center">
				...
				<tr>
					<td>来电号码:</td>
					<td><input  id="customertelephone" type="text" class="easyui-validatebox" name="telephone"
						required="true" /></td>
					<td>客户编号:</td>
