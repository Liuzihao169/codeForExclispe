...
<script type="text/javascript">
	...
	function doExport(){
		...
		if(arr.length==0){
			$.messager.alert('提示信息','请选择你要导出的行','warning');  
		}
		else{
			var array = new Array();
			$.messager.confirm('确认','您确认想要导出这些行吗？',function(r){    
			    if (r){
			    	for(var i=0;i<arr.length;i++){
			    		array.push(arr[i].id);
			    	}
			    	var ids=array.join(",");
			    	location.href="${pageContext.request.contextPath}/subareaAction_exportXls?ids="+ids;
			    }    
			});  
		}
	}
	...
	$(function(){
		...
		//将表格里面的内容转换成json 然后会被当作参数动态刷新过去
					...
					<tr>
						...
						<td>
							<input class="easyui-combobox" name="region.id"  
    							data-options="valueField:'id',mode:'remote',textField:'name',url:'${pageContext.request.contextPath}/regionAction_findRegionAjax'" />  
						</td>
					</tr>
					<tr>
						<td>关键字</td>
