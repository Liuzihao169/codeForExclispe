...
<body>
	...
	<div style="width: 380px; margin: 0 auto; margin-top: 50px;">
		<ul class="pagination" style="text-align: center; margin-top: 10px;">
		 ...
		 <c:if test="${pageBean.currentpage==1}">
			<li class="disabled"><a href="javascript:void(0)" aria-label="Previous"><span
					...
					<c:if test="${pageBean.currentpage!=1}">
			<li ><a href="${pageContext.request.contextPath}/product?currentpage=${pageBean.currentpage-1}&cid=${cid}&method=productlistbycid" 
		...
		<c:forEach begin="1" end="${pageBean.totalpage}" var="page">
			...
			<c:if test="${pageBean.currentpage!=page}">
				<li ><a href="${pageContext.request.contextPath}/product?currentpage=${page}&cid=${cid}&method=productlistbycid">${page}</a></li>
			</c:if>
			<!-- <li><a href="#" aria-label="Next"> <span aria-hidden="true">&raquo;</span>
		     ...
		     <c:if test="${pageBean.currentpage==pageBean.totalpage}">
		     	...
		     	<c:if test="${pageBean.currentpage!=pageBean.totalpage}">
		     	<li><a href="${pageContext.request.contextPath}/product?currentpage=${pageBean.currentpage+1}&cid=${cid}&method=productlistbycid" aria-label="Next"> <span aria-hidden="true">&raquo;</span>
		     	</c:if> 
		</ul>
	</div>
	<!-- 分页结束 -->
